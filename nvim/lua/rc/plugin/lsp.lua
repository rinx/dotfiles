...
lsp.efm.setup(core.merge(default_options, {filetypes = {"bash", "dockerfile", "markdown", "proto", "rego", "sh", "vcl"}, init_options = {codeAction = true, completion = true, documentFormatting = true, documentSymbol = true, hover = true}, settings = {languages = {bash = {{lintCommand = "shellcheck -f gcc -x", lintSource = "shellcheck", lintFormats = {"%f:%l:%c: %trror: %m", "%f:%l:%c: %tarning: %m", "%f:%l:%c: %tote: %m"}}}, dockerfile = {{lintCommand = "hadolint --no-color", lintFormats = {"%f:%l %m"}}}, markdown = {{lintCommand = "deno run --allow-env --allow-read --allow-sys --no-lock npm:markdownlint-cli@latest -s -c ~/.dotfiles/markdownlintrc", lintIgnoreExitCode = true, lintStdin = true, lintFormats = {"%f:%l %m", "%f:%l:%c %m", "%f: %l: %m"}}, {hoverCommand = "excitetranslate.clj", hoverStdin = true}}, proto = {{lintCommand = "buf lint --path"}}, rego = {{lintCommand = "opa check --strict", lintIgnoreExitCode = true, lintFormats = {"%m: %f:%l: %m", "%f:%l: %m"}}}, sh = {{lintCommand = "shellcheck -f gcc -x", lintSource = "shellcheck", lintFormats = {"%f:%l:%c: %trror: %m", "%f:%l:%c: %tarning: %m", "%f:%l:%c: %tote: %m"}}}, vcl = {{lintCommand = "falco -vv lint ${INPUT} 2>&1", lintIgnoreExitCode = true, lintFormats = {"%E\240\159\146\165 %m", "%E\240\159\148\165 [ERROR] %m", "%W\226\157\151\239\184\143 [WARNING] %m", "%I\240\159\148\136 [INFO] %m", "%Zin %f at line %l, position %c", "%-G%.%#"}}}}, lintDebounce = 3000000000}}))
