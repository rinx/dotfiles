...
local function initialize()
  return vim.fn["skkeleton#config"]({globalDictionaries = {"~/.SKK-JISYO.L"}, immediatelyDictionaryRW = true, keepState = true, selectCandidateKeys = "asdfjkl", setUndoPoint = true, showCandidatesCount = 4, userDictionary = "~/.dotfiles.local/skk-jisyo", databasePath = "~/.cache/nvim/skkeleton.db", sources = {"deno_kv", "skk_dictionary"}, eggLikeNewline = false, registerConvertResult = false})
end
