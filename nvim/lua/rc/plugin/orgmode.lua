...
local function _7_(exporter)
  local current = vim.api.nvim_buf_get_name(0)
  local target = (vim.fn.fnamemodify(current, ":p:r") .. ".md")
  local cmd = {"pandoc", current, "--from=org", "--to=gfm"}
  local on_success
