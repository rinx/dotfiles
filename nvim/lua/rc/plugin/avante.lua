...
mcphub.setup({config = vim.fn.expand("~/.nix-profile/config/mcp-servers.json"), extensions = {avante = {make_slash_commands = true}}, native_servers = {["org-mode"] = orgmode_server, ["org-roam"] = orgroam_server}, auto_approve = false})
do
  local toggle
  local function _20_()
    return (vim.g.mcphub_auto_approve == true)
  end
  local function _21_(state)
    if state then
  ...
  toggle = Snacks.toggle.new({id = "mcphub_auto_approve", name = "MCPHub auto_approve", get = _20_, set = _21_})
