...
avante.setup({provider = "copilot", behavior = {auto_apply_diff_after_generation = true, enable_cursor_planning_mode = true, auto_set_keymaps = false, auto_suggestions = false}, providers = {copilot = {model = "claude-3.7-sonnet"}, ["copilot-gemini-2.5-pro"] = {__inherited_from = "copilot", model = "gemini-2.5-pro"}, ["copilot-gpt-4.1"] = {__inherited_from = "copilot", model = "gpt-4.1"}, ["copilot-gpt-4o"] = {__inherited_from = "copilot", model = "gpt-4o"}, bedrock = {model = "apac.anthropic.claude-sonnet-4-20250514-v1:0", aws_profile = "bedrock-claude-sonnet4", aws_region = "ap-northeast-1", context_window = 200000, extra_request_body = {temperature = 0.75, max_tokens = 64000}}}, hints = {enabled = false}, file_selector = {provider = "snacks"}, mappings = {ask = "<leader>Aa", edit = "<leader>Ae", refresh = "<leader>Ar", focus = "<leader>Af", stop = "<leader>AS", toggle = {default = "<leader>At", debug = "<leader>Ad", hint = "<leader>Ah", suggestion = "<leader>As", repomap = "<leader>AR"}, files = {add_current = "<leader>Ac", add_all_buffers = "<leader>AB"}, select_model = "<leader>A?", select_history = "<leader>AH"}, system_prompt = _20_, custom_tools = _21_, disabled_tools = {"create_dir", "create_file", "delete_dir", "delete_file", "list_files", "python", "rag_search", "read_file", "rename_dir", "rename_file", "search_files", "web_search"}})
