...
return use({ast_grep = {filetypes = {"bash", "c", "cpp", "css", "fennel", "go", "html", "java", "javascript", "javascriptreact", "json", "kotlin", "lua", "nix", "python", "rust", "typescript", "typescriptreact", "yaml"}}, bashls = {settings = {bashIde = {shfmt = {caseIndent = true}}}}, buf_ls = {}, clojure_lsp = {}, cssls = {}, cue = {}, dagger = {}, denols = {init_options = {lint = true, unstable = true}, root_dir = root_pattern("deno.json", "deno.jsonc", "deps.ts")}, docker_compose_language_service = {}, dockerls = {}, efm = {filetypes = {"dockerfile", "jjdescription", "gitcommit", "proto", "rego", "vcl", "yaml.github-actions"}, init_options = {codeAction = true, completion = true, documentFormatting = true, documentSymbol = true, hover = true}, settings = {languages = {dockerfile = {{lintCommand = "hadolint --no-color", lintSource = "efm/hadolint", lintAfterOpen = true, lintFormats = {"%f:%l %m"}}}, gitcommit = {{lintCommand = "gitlint --config ~/.gitlint", lintStdin = true, lintSource = "efm/gitlint", lintAfterOpen = true, lintFormats = {"%l: %m: \"%r\"", "%l: %m"}}}, jjdescription = {{lintCommand = "gitlint --config ~/.gitlint", lintStdin = true, lintSource = "efm/gitlint", lintAfterOpen = true, lintFormats = {"%l: %m: \"%r\"", "%l: %m"}}}, proto = {{lintCommand = "buf lint --path", lintSource = "efm/buf-lint", lintAfterOpen = true, lintFormats = {"%f:%l:%c:%m"}, lintSeverity = 2, rootMarkers = {"buf.yaml"}}}, rego = {{lintCommand = "opa check --strict", lintIgnoreExitCode = true, lintSource = "efm/opa-check-strict", lintAfterOpen = true, lintFormats = {"%m: %f:%l: %m", "%f:%l: %m"}}}, ["yaml.github-actions"] = {{lintCommand = "actionlint -no-color -oneline -stdin-filename \"${INPUT}\" -", lintStdin = true, lintSource = "efm/actionlint", lintAfterOpen = true, lintFormats = {"%f:%l:%c: %.%#: SC%n:%trror:%m", "%f:%l:%c: %.%#: SC%n:%tarning:%m", "%f:%l:%c: %.%#: SC%n:%tnfo:%m", "%f:%l:%c: %m"}, requireMarker = true, rootMarkers = {".github/"}}}}, lintDebounce = "300ms"}}, fennel_ls = {root_dir = root_pattern(".nfnl.fnl", "flsproject.fnl")}, fortls = {cmd = alter_cmd({"fortls", "--notify_init", "--hover_signature", "--hover_language=fortran", "--use_signature_help"}, "fortls")}, gleam = {}, gopls = {settings = {gopls = {usePlaceholders = true, analyses = {shadow = true, useany = true, unusedvariable = true}, hints = {assignVariableTypes = true, compositeLiteralFields = true, compositeLiteralTypes = true, constantValues = true, functionTypeParameters = true, parameterNames = true, rangeVariableTypes = true}, staticcheck = true, vulncheck = "Imports", gofumpt = true}}}, harper_ls = {cmd = alter_cmd({"harper", "--stdio"}, "harper"), filetypes = {"asciidoc", "clojure", "gitcommit", "go", "jjdescription", "html", "markdown", "nix", "plaintex", "plaintext", "sh", "tex", "text", "toml", "typescript", "typst"}}, helm_ls = {cmd = alter_cmd({"helm_ls", "serve"}, "helm-ls")}, jqls = {cmd = alter_cmd({"jq-lsp"}, "jq-lsp")}, jsonls = {settings = {json = {schemas = schemastore.json.schemas()}}}, lua_ls = {cmd = alter_cmd({"lua-language-server"}, "lua-language-server")}, marksman = {}, nginx_language_server = {cmd = alter_cmd({"nginx-language-server"}, "nginx-language-server")}, nil_ls = {}, nixd = {}, pylsp = {cmd = alter_cmd({"pylsp"}, "python313Packages.python-lsp-server")}, regal = {init_options = {enableDebugCodelens = true, evalCodelensDisplayInline = true}}, rust_analyzer = {cmd = alter_cmd({"rust-analyzer"}, "rust-analyzer")}, terraformls = {init_options = {experimentalFeatures = {validateOnSave = true, prefillRequiredFields = true}}}, tflint = {}, tinymist = {}, tsgo = {cmd = {"tsgo", "--lsp", "--stdio"}, filetypes = {"javascript", "javascriptreact", "javascript.jsx", "typescript", "typescriptreact", "typescript.tsx"}, root_dir = root_pattern("package.json", "tsconfig.json", "jsconfig.json")}, yamlls = {settings = {yaml = {format = {enable = false}, schemas = _12_, validate = true}, single_file_support = true}}})
