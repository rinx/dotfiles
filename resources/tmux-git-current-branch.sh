...
if git_status=$(cd $1 && git status 2>/dev/null ); then
    ...
    git_info="#[underscore]#[bg=black,fg=blue][git:${git_branch}]#[default]${state}"
else
    git_info=""
fi
...
if k8s_context="$(kubectl config current-context 2>/dev/null)"; then
    ...
    k8s_info="#[underscore]#[bg=black,fg=yellow][k8s:${k8s_context}/${k8s_ns}]#[default]"
else
    k8s_info=""
fi
...
echo "$directory $k8s_info $git_info"
